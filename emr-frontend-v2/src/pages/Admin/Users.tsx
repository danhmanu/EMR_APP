...
export default function Users(): JSX.Element{
  ...
  const cols = [
    ...
    { title: 'Role', render:(v:any,row:any)=> row.role?.name },
    { title: 'Khoa/Phòng', render:(v:any,row:any)=> departments.find((d:any)=> d.id === row.departmentId)?.name || '-' },
    { title: 'Active', render:(v:any,row:any)=>(<Switch checked={row.isActive} onChange={async ()=>{ try{ const res = await toggleUser.mutateAsync(row.id); if(res?.success) message.success('Cập nhật thành công'); else message.error('Cập nhật thất bại') }catch(e){ message.error('Cập nhật thất bại') } }} />) },
    { title: 'Hành động', render:(v:any,row:any)=>(<><Button type="link" onClick={()=>{ setEditing(row); form.setFieldsValue(row); setOpen(true) }}>Sửa</Button></>) }
  ]
  ...
  return (
    <div>
      <Card title="Người dùng">
        <Button type="primary" onClick={()=>{ setEditing(null); form.resetFields(); setOpen(true) }} style={{marginBottom:12}}>Thêm người dùng</Button>
        <Table dataSource={rows} columns={cols} rowKey={r=>r.id} loading={createUser.isPending || updateUser.isPending || toggleUser.isPending} />
      </Card>
